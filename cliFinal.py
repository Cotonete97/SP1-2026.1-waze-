...
VELOCIDADE_MAX_MAPA_MS = 80 / 3.6 #Converte a velocidade máxima geral (80 km/h) para metros por segundo (m/s) dividindo por 3.6
...
FRENAGEM_MS2    = 3.0 #Taxa de desaceleração (m/s²) de um carro normal freando
ACELERACAO_MS2  = 2.0 #Taxa de aceleração (m/s²) de um carro normal
ESPERA_SEMAFORO_S = 30.0 #Tempo fixo médio que o carro fica parado no sinal vermelho (em segundos)
VEL_LOMBADA_KMH = 20.0 #Velocidade alvo que o carro precisa atingir para passar na lombada
ZONA_LOMBADA_M  = 30.0 #Tamanho total da área da lombada (o carro freia 15m antes e acelera 15m depois)
...
def atualizar_pesos_do_grafo():
    ...
    for node, data in G.nodes(data=True):  # varre todos os nós (cruzamentos) e seus dados no mapa
        if node in edicoes_usuario['removidos']:
            continue  # pula se o usuário tiver removido esse nó
        if data.get('highway') == 'traffic_signals':  # se for um semáforo original do mapa
            if node not in sem60:
                sem30.add(node)  # adiciona como 30s se não estiver no conjunto de 60s
        if data.get('traffic_calming') in ['bump', 'hump']:  # se for lombada original do mapa
            if node not in lom60 and node not in lom80:
                lom40.add(node)  # adiciona no conjunto de 40km/h se não estiver nos conjuntos de 60 ou 80km/h

    for u, v, key, data in G.edges(keys=True, data=True):  # varre todas as arestas (ruas) entre os nós 'u' e 'v'
        distancia_m = data.get('length', 0)  # distância do trecho em metros (d); se não tiver, assume 0
        # tenta pegar a velocidade editada pelo usuário ou usa 30 km/h por padrão
        vel_kmh = edicoes_usuario['velocidades'].get((u, v, key), 30)
        vel_ms  = max(0.1, vel_kmh / 3.6)  # converte km/h para m/s (max(0.1, x) impede que a velocidade seja zero)
        tempo_base = distancia_m / vel_ms  # tBase = d/v
        penalidade_total = 0  # inicializa soma de penalidades
...
def heuristica_tempo(n1, n2): #função heuristica(atual, fim)
    ...
    # Calcula Pitágoras para achar a distância em linha reta e multiplica por 111139
    # para converter os graus de mapa para metros reais
    distancia_reta = ((x1 - x2)**2 + (y1 - y2)**2)**0.5 * 111139
    # h(n) em tempo = Distância / Velocidade Máxima Absoluta
    return distancia_reta / VELOCIDADE_MAX_MAPA_MS  #h(n) em tempo = Distância / Velocidade Máxima Absoluta
    ...
    while fila:#enquanto tiver nós para analisar
        ...
        for vizinho in G.neighbors(atual):#"G.neighbors(atual)" me dá os vizinhos ligados ao atual
            ...
            if vizinho not in g_custo or novo_g < g_custo[vizinho]:
                g_custo[vizinho] = novo_g  # atualizo dist[vizinho]
                f = novo_g + heuristica_tempo(vizinho, fim)  # f(n) = g(n) + h(n)
                pais[vizinho] = atual  # atualizo prev[vizinho]
                heapq.heappush(fila, (f, vizinho))  # adiciono o vizinho na fila de processamento

    caminho, atual = [], fim  # inicia a reconstrução a partir do destino
    while atual in pais:  # enquanto existir um nó de onde eu vim
        caminho.append(atual)  # adiciona o nó à rota
        atual = pais[atual]  # volta para o nó anterior
    caminho.append(inicio)  # adiciona o nó inicial por último
    caminho.reverse()  # inverte a lista para ordem Origem → Destino
